...
class ACombo:
	...
	self.stage_dims = the dimensions of the current stage on [x_max, y_max, x_min, y_min]
	self.drawing = the tkinter drawing if made
	'''
	...
	def __init__(self, combotext, char_dict, stage):
		...
		match str(self.stage).split('.')[1]:
			case 'POKEMON_STADIUM':
				#yoshis as stand in, cant find online
				self.stage_dims = [173.60, 168.00, -175.70, -91.00]
			case 'DREAM_LAND_N64':
				self.stage_dims = [255, 250, -255, -123]
			case 'YOSHIS_STORY':
				self.stage_dims = [173.60, 168.00, -175.70, -91.00]
			case 'FOUNTAIN_OF_DREAMS':
				self.stage_dims = [198.75,202.50, -198.75,-146.25]
			case 'BATTLEFIELD':
				self.stage_dims = [224, 200,-224,-108.8]
			case 'FINAL_DESTINATION':
				self.stage_dims = [246.00,188.00,-246.00,-140,00]
			case other:
				self.stage_dims = None
...
def main_specific(dire, character, stage, hits, vs_char=['all']):
	...
	newcombolist = [x for x in newcombolist if (x.moves != [])]
	if hits != 0:
